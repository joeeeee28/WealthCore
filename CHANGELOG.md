...
## v1.6.0 — Demo & sandbox data environments (centralised `WEALTHCORE_DATA_ENVIRONMENT`)
- **Central data environment**: one switch `WEALTHCORE_DATA_ENVIRONMENT` = `DEMO` (default) | `FINVU_SANDBOX` | `SETU_SANDBOX` | `PRODUCTION`, validated at startup; invalid values fail configuration validation. Resolution is centralised in `server/aa/data-environment.js` (`getDataEnvironment()` / `getActiveProvider()` / `resolveAAPProvider()`); environment-selection logic is never duplicated.
- **Production safety**: PRODUCTION never falls back to the demo environment — automatically or silently. Without configured Finvu/Setu credentials, validation fails and resolution returns a NOT-configured PRODUCTION stub; `AA_PROVIDER=demo|mock` under PRODUCTION is a hard misconfiguration; demo mutations return `DEMO_DISABLED_IN_PRODUCTION`.
- **Demo provider** (`server/aa/providers/demo.js`): full AAProvider contract (createConsent → approve → requestFIData → markDataReady → getFIData → normalize), `mode: DEMO`, `configured: true`, `synthetic: true`, `requiresCredentials: false`, `status: READY`; registered in the AA provider registry as `demo` without touching mock/finvu/setu.
- **Deterministic synthetic fixtures** (`server/aa/fixtures/demo-data.js`, seed `wealthcore-demo-v1`): 7 AA accounts (incl. 2 loan liabilities), 12 holdings, 3 retirement assets, 3 insurance records, 6 goals, 4 budgets, exactly 180 transactions (~6 months, ids `DEMO-TXN-0001…0180`). Every record labelled `source='DEMO'`, `synthetic=true`, DB `is_demo=1`, `provider='demo'`, `DEMO-*` refs.
- **Demo operations** (`server/lib/demo.js`): `GET /api/v1/demo/status`, `POST /api/v1/demo/load|refresh|reset` through the existing AA sync pipeline; user-scoped isolation; idempotent load/refresh (zero duplicates); reset touches only the calling user's demo rows. Environment introspection: `GET /api/v1/data-environment`, `GET /api/v1/data-environments` (public, labels/flags only).
- **Financial correctness**: nothing hard-coded — demo values flow through the existing net-worth/portfolio/reconciliation engines; invariants verified by tests (opening+credits−debits=closing; qty×price=value; assets−liabilities=net worth; portfolio & debt reconciliation).
- **UI**: persistent environment indicator (🧪 DEMO MODE / 🔬 FINVU SANDBOX / 🔬 SETU SANDBOX / 🔐 PRODUCTION) on shell + auth screens; DEMO banner “DEMO DATA — All financial information shown is synthetic.”; Settings → Demo wealth profile status + Load/Refresh/Reset controls.
- Bug fix: `runAASync` now approves a consent it implicitly creates (matching its documented contract), which preserves the explicit-DB-consent path used by real providers.
- Docs: new `docs/DEMO_DATA.md`, `docs/DATA_ENVIRONMENTS.md`; README/.env.example/docs index updated.
- Tests: **125 → 191** (new suites: environment-config, provider-registry, demo-provider, demo-normalization, demo-seed, demo-financial-calculations, demo-reconciliation). `lint`, `lint:client`, `config:check`, `audit:secrets`, `audit:deps` all pass.
- Existing **Finvu and Setu functionality preserved**: registry entries/adapters unchanged; Setu sandbox/auth/webhook suites unaffected; all prior tests still pass unchanged.
