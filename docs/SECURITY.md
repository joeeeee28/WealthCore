...
- **No secrets in source.** Verified with a scan for `API_KEY|SECRET|PASSWORD|TOKEN|PRIVATE_KEY` across `server/` and `public/` — no hard-coded literals found (`npm run audit:secrets`).
